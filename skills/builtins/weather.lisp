...
;;; --- Sunrise/Sunset (Ephemera) ---

(defun calculate-sunrise-sunset (latitude longitude)
  "Calculate sunrise and sunset as decimal hours (0-24) for latitude/longitude.
   Uses simplified algorithm. Returns (values sunrise-hours sunset-hours)."
  (let* ((doy (multiple-value-bind (s mi h d mo y dow dstp tz)
               (decode-universal-time (get-universal-time))
             (declare (ignore s mi h dow dstp tz))
             (let ((days-in-months (vector 0 31 28 31 30 31 30 31 31 30 31 30 31)))
               (when (and (= (mod y 4) 0) (or (not (= (mod y 100) 0)) (= (mod y 400) 0)))
                 (setf (aref days-in-months 2) 29))
               (loop for i from 1 below mo sum (aref days-in-months i) into days
                     finally (return (+ days d))))))
         (frac (* 2 pi (/ (- doy 1) 365.0d0)))
         (decl (asin (* 0.39445 (sin frac))))
         (lat-rad (/ (* latitude pi) 180.0d0))
         (arg (min 1 (max -1 (- (* (tan lat-rad) (tan decl))))))
         (h (acos arg)))
    (let ((sunrise (- 12 (/ (* h 180) pi 15.0d0)))
          (sunset (+ 12 (/ (* h 180) pi 15.0d0))))
      (values (mod (max 0 sunrise) 24)
              (mod (max 0 sunset) 24)))))

(defun ephemera-report (&key (city nil) (stream t))
  "Print sunrise/sunset times in human-friendly format."
  (let* ((city-name (or city
                        (crichton/config:config-section-get :weather :city)
                        "Victoria"))
         (data (fetch-weather-json city-name))
         (feature (aref (jget data "features") 0))
         (geom (gethash "geometry" feature))
         (coords (gethash "coordinates" geom)))
    (destructuring-bind (lon lat) (coerce coords 'list)
      (let ((city-label (jen (jget feature "properties") "name")))
        (format stream "~&Ephemera for ~A~%" city-label)
        (format stream "  Location: ~,2F°N, ~,2F°W~%" lat (- lon))
        (multiple-value-bind (sr ss) (calculate-sunrise-sunset lat lon)
          (format stream "  Sunrise: ~2,'0D:~2,'0D~%" 
                  (floor sr)
                  (round (* (mod sr 1) 60)))
          (format stream "  Sunset:  ~2,'0D:~2,'0D~%" 
                  (floor ss)
                  (round (* (mod ss 1) 60)))
          (format stream "  Daylight: ~,1F hours~%" 
                  (- ss sr)))))))
